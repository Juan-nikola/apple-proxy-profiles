...
#### 1. 在 Sub-Store 建立 12 个手动 collection
...
新版界面中创建一个文件后，在“文件操作”里添加“脚本操作/Script Operator”，将 generator URL 填入脚本操作的“远程链接”；文件本身保持空内容，不要把 generator URL 填到“文件 → 远程”的源文件链接，否则 Preview 只会显示 JavaScript 源码。然后点击 Preview 和保存。仓库的 49 个 canonical task 已按同样字段组织；参数中的私密节点 URL 只填写在你自己的 Sub-Store，不要回填到 README 或 GitHub。
...
| `ipv6Mode` | DNS 和连接选择 IPv4/IPv6 的方式 | `auto`、`ipv4-only` | 所有 40 个配置任务均为 `ipv4-only` |
...
| `nodeErrorMode` | sing-box/Hiddify 遇到不兼容节点时的处理 | `strict`、`compatible` | `strict` |
...
示例展开 Surge、sing-box 和 Hiddify；新建完整 policy 包含 `anywhere`、`egern`、`shadowrocket`、`surge`、`sing-box`、`happ`、`v2rayn`、`v2box`、`clash`、`incy`、`hiddify` 十一个客户端层，每层都必须完整填写 13 个 target。业务组名称固定为 `🤖 AI 专用`、`🐙 GitHub`、`📺 YouTube`、`🎬 海外流媒体`、`💬 海外社交`、`🍎 Apple`、`🪟 Microsoft`、`🇨🇳 国内平台`、`🌍 海外游戏`、`🎮 游戏连接`、`⬇️ 下载/P2P`、`🧭 DNS 与规则下载`、`漏网之鱼`。
...
| 11 | Hiddify Next | Hiddify 官方跨平台客户端，使用兼容的 sing-box JSON |
...
六个平台生成器读取独立的 `apple-proxy-hiddify` 节点池，参数和任务见 [Sub-Store 指南](docs/substore-two-layer-setup.md)，客户端导入限制见 [Hiddify README](clients/hiddify/README.md)。配置按共享业务规则分流：已知国内域名使用国内 DNS，已知海外域名使用代理 DoH；未知域名先经代理 DoH 解析，再按 `ChinaIP` 和 `漏网之鱼` 判断出口。当前没有国内/国外两套 DNS 答案比较功能。

Hiddify v4.1.1 的内核与本项目 sing-box 1.14 配置有差异，Hiddify 使用独立 source JSON 规则，拒绝 Snell。完整配置必须通过保留 DNS/route 的 raw config 路径运行；普通订阅导入会抽取节点并重建路由，不能据此宣称项目规则已生效。目前未完成六平台真实设备导入及分流验收，不保证每个目标都能自动正确归类。

## 统一行为

- unified policy 使用 schema v3 按客户端分层；每层嵌套 schema v2 的完整 13 项 targets，当前包含十一个 active client，schema v1/v2 仅作为兼容读取器。
...
当前 canonical catalog 为 12 个手动 collection、49 个 File task：
...
| Hiddify Next | 6 | Android/iPhone/iPad/macOS/Windows/Linux JSON Config |
...
后台只保留上述 12 个手动 collection 和 49 个 canonical task；删除旧的 OneXray 对象及其任务。不要在 Sub-Store 使用 `subscriptionTags` 自动识别，所有节点来源通过 collection 手动勾选。
