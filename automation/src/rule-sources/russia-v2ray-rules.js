...
export const RUSSIA_ALL_DOMAIN_ENTRY_BUDGET = 100_000;

function annotate(parsed, sourceId) {
  const categories = parsed.categories.map((category) => ({ ...category, region: "ru", categoryId: category.id }));
  const entries = parsed.entries.map((entry) => ({ ...entry, region: "ru", categoryId: entry.categoryId ?? entry.category }));
  const allDomainCount = entries.filter(({ categoryId }) => categoryId === "TEST_ONLY_ALL_DOMAIN"
    || categoryId === "all"
    || categoryId === "geolocation-!cn").length;
  if (allDomainCount > RUSSIA_ALL_DOMAIN_ENTRY_BUDGET) {
    throw new Error(`External source ${sourceId}: all-domain category exceeds entry budget`);
  }
  return { ...parsed, entries, categories };
}

export function parseRussiaV2rayRules({ text, sourceId }) {
