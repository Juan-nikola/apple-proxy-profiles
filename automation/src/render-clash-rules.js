...
const OMITTED = new Set([RULE_KIND.geoip, RULE_KIND.logicalAnd, RULE_KIND.logicalOr]);

export function renderClashRuleSource({ source, parsed, fetched, upstream, capabilities = {} }) {
  ...
  for (const entry of parsed.entries) {
    ...
    } else if (entry.kind === RULE_KIND.processName && capabilities.processName !== false) {
      payload.push("PROCESS-NAME," + entry.value);
    } else if (entry.kind === RULE_KIND.processName || OMITTED.has(entry.kind)) {
  ...
  return Object.freeze({
    content: header.join("\n") + "\n" + renderYaml({ payload }),
    counts: Object.freeze({ input: parsed.diagnostics.candidateCount, parsed: parsed.diagnostics.parsedCount, output: payload.length, omitted: Object.values(omittedByKind).reduce((sum, value) => sum + value, 0), omittedByKind: Object.freeze(omittedByKind), unsupportedByReason: Object.freeze(Object.fromEntries(Object.entries(omittedByKind).map(([kind, count]) => [`unsupported-${kind.replace(/[A-Z]/gu, (letter) => `-${letter.toLowerCase()}`)}`, count]))) }),
  });
}
