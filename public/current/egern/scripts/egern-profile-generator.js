var EgernProfileBundle = (() => {
  ...
  function validateSharedGraph(input) {
    ...
    if (root.kind !== GROUP_KIND.primary || root.strategy !== STRATEGY.select || root.nodeFilter !== null || root.test !== null || root.hidden !== void 0 || root.defaultChoice !== void 0) {
  ...
  function validatedOptions(options) {
    ...
    if (!baseDescriptor || "get" in baseDescriptor || "set" in baseDescriptor || !FRONTIER_CHANNELS.some((channel) => base === `${PUBLIC_RULE_ROOT}/${channel}`) || !adblockDescriptor || "get" in adblockDescriptor || "set" in adblockDescriptor || adblockMode !== "off" && adblockMode !== "full") {
      throw new Error("Invalid Egern public rule base");
    }
...
`;
  ...
  function validShadowTls(value) {
    return isPlainObject2(value) && Object.keys(value).every((key) => key === "password" || key === "sni") && isNonblankOpaqueString(value.password) && (!hasOption(value, "sni") || isNonblankString(value.sni));
  }
  ...
  function egernTlsReason(node, {
    ...
    const reality = node["reality-opts"];
    if (reality !== void 0) {
      if (node.tls === false || !allowReality || !isPlainObject2(reality) || !isRealityPublicKey(reality["public-key"])) {
  ...
  function validHttpTransportOptions(options) {
    if (!isPlainObject2(options)) return false;
  ...
  function egernVmessVlessTransportReason(node) {
    ...
    if (network === "ws") {
      ...
      if (!isPlainObject2(options) || Object.keys(options).some((key) => !["path", "headers"].includes(key)) || !validPath(options.path) || hasOption(options, "headers") && !validHeaders(options.headers) || hasOption(node, "reality-opts")) {
    ...
    if (network === "grpc") {
      ...
      if (!isPlainObject2(options) || Object.keys(options).some((key) => !["grpc-service-name", "grpc-mode", "user-agent"].includes(key)) || hasOption(options, "grpc-service-name") && !isNonblankString(options["grpc-service-name"]) || hasOption(options, "user-agent") && !isNonblankString(options["user-agent"]) || hasOption(options, "grpc-mode") && options["grpc-mode"] !== "gun") {
        return "unsupported-egern-transport";
      }
      return null;
    }
  ...
  function egernWireGuardReason(node) {
    if (hasOption(node, "peers") && (!Array.isArray(node.peers) || node.peers.length !== 1 || !isPlainObject2(node.peers[0]))) {
  ...
  function headersInNode(node) {
    ...
    for (const key of ["ws-opts", "h2-opts", "http-opts"]) {
      if (isPlainObject2(node[key]) && hasOption(node[key], "headers")) values.push(node[key].headers);
    }
    return values;
  }
  function hasHttp2HostConflict(node) {
    const options = node["h2-opts"];
    if (!isPlainObject2(options) || !hasOption(options, "host") || !isPlainObject2(options.headers)) return false;
    const host = Array.isArray(options.host) ? options.host[0] : options.host;
  ...
  function egernCommonReason(node, protocol2) {
    if (!isPlainObject2(node) || !isNonblankString(node.name) || !isNonblankString(node.server) || !isValidPort(node.port)) return "invalid-egern-node-shape";
  ...
  function egernNodeExclusionReason(node) {
    ...
    if (protocol2 === "trojan") {
      ...
      if (network === "ws") {
        const options = node["ws-opts"];
        if (!isPlainObject2(options) || Object.keys(options).some((key) => !["path", "headers"].includes(key)) || !validPath(options.path) || hasOption(options, "headers") && (!validHeaders(options.headers) || Object.keys(options.headers).some((key) => key.toLowerCase() !== "host"))) {
          return "unsupported-egern-transport";
        }
  ...
  function toEgernProxy(node, options = {}) {
    const protocol2 = protocolForError(node);
    try {
      return renderEgernProxy(node, options);
    } catch {
      throw new Error(`Egern cannot render protocol: ${protocol2}`);
    }
  }
  ...
  var EXTERNAL_NODE_POLICY = /^.+ · (?:SS|SSR|Snell|VMess|VLESS|Trojan|AnyTLS|Hy2|Tuic|SOCKS5|HTTP|SSH|WireGuard)(?:｜|·|$)/u;
  var INTERACTIVE_POLICY_VALUES = /* @__PURE__ */ new Set(["\u{1F680} \u8282\u70B9\u9009\u62E9", "DIRECT"]);
  ...
  function canonicalCandidates(fields, schema) {
    ...
    if (first === "\u{1F680} \u8282\u70B9\u9009\u62E9" && schema.defaultChoice === "DIRECT") {
      ...
      if (remainder.includes("DIRECT")) {
        return ["DIRECT", "\u{1F680} \u8282\u70B9\u9009\u62E9", ...remainder.filter((candidate) => candidate !== "DIRECT")];
      }
    }
  ...
  function reconstructGroups(policyGroups) {
    ...
    let stripped = [];
    let shared = [];
    for (const record2 of policyGroups) {
      if (record2 === null || typeof record2 !== "object" || Array.isArray(record2)) {
        throw new Error("Invalid Egern policy group schema");
      }
      const wrapperKeys = Object.keys(record2);
      if (wrapperKeys.length !== 1 || !["select", "auto_test", "fallback"].includes(wrapperKeys[0])) {
        throw new Error("Invalid Egern policy group schema");
      }
      const type = wrapperKeys[0];
      const originalFields = record2[type];
      ...
      if (fields.urls !== void 0) {
        ...
        if (nodeSubscriptionUrl !== void 0 && nodeSubscriptionUrl !== fields.urls[0]) {
          throw new Error("Invalid Egern policy group subscription URL");
        }
    ...
    const validPreset = GROUP_TEST_PRESETS.some((preset) => stripped.every((record2) => {
      const [type] = Object.keys(record2);
  ...
  function applyProxyQuicOverrides(rendered, shared, quicMode) {
    ...
    return rendered.map((record2) => {
      ...
      const fields = record2[type];
      if (!proxyDefaults.has(fields.name)) return record2;
      return { [type]: { ...fields, block_quic: true } };
    });
  }
  var POLICY_GROUP_BY_TARGET = Object.freeze(Object.fromEntries(UNIFIED_POLICY_TARGETS.map(({ id, label }) => [id, label])));
  function policyValue(record2) {
    if (!record2 || typeof record2.resolved !== "string") return null;
    if (record2.resolved === "FOLLOW") return "\u{1F680} \u8282\u70B9\u9009\u62E9";
    return record2.resolved;
  }
  ...
  function renderEgernProfileFromOptions(options, nodes, {
    onDiagnostics,
  ...
  function parseNodeReference(target) {
    ...
    if (body.length === 0 || LINE_TERMINATOR2.test(body)) {
    ...
    if (name.length === 0 || mode === "exact" && name.trim() !== name || LINE_TERMINATOR2.test(name)) {
      throw invalid("node name is empty or contains a line break");
    }
    return freeze2(mode === "fuzzy" ? { mode, query: name, protocol: protocol2 } : { mode, name, protocol: protocol2 });
  }
  ...
  function resolveUnifiedPolicy({
    ...
    return freeze3({ targets, fixedNodes, warnings: [] });
  }
  ...
  async function operator(input, targetPlatform, context = {}) {
    ...
    const policyResolution = policy === null ? null : resolveUnifiedPolicy({
      policy,
