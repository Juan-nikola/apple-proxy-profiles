var HiddifyConfigBundle = (() => {
  ...
  // src/options.js
  var REQUIRED = ["output", "type", "name", "subscriptionName", "platform"];
  var DEFAULTS = Object.freeze({ channel: "current", dnsMode: "stable", chinaDns: "alidns", globalDns: "cloudflare", blockMode: "balanced", quicMode: "proxy-block", ipv6Mode: "ipv4-only", autoGroupMode: "auto", clientChain: "off", profileMode: "light", adblockMode: "off", nodeErrorMode: "strict" });
  var PLATFORMS = /* @__PURE__ */ new Set(["android", "iphone", "ipad", "macos", "windows", "linux"]);
  ...
  function requiredString(raw, key) {
    const value = raw[key];
    if (typeof value !== "string" || !value || value.trim() !== value || /[\r\n]/u.test(value)) throw new Error(`Option '${key}' must be a non-empty single-line string`);
    return value;
  }
  function enumValue(raw, key) {
    const value = raw[key] === void 0 ? DEFAULTS[key] : raw[key];
    if (typeof value !== "string" || !OPTION_VALUES[key]?.includes(value)) throw new Error(`Option '${key}' has an unsupported value`);
    return value;
  }
  function parseHiddifyOptions(raw) {
    if (!raw || typeof raw !== "object" || Array.isArray(raw)) throw new TypeError("hiddify options must be an object");
    for (const key of Object.keys(raw)) if (!key.startsWith("_") && ![...REQUIRED, ...Object.keys(DEFAULTS)].includes(key)) throw new Error(`Unknown Hiddify option: ${key}`);
    for (const key of REQUIRED) if (!Object.hasOwn(raw, key)) throw new Error(`Option '${key}' is required`);
    ...
    if (!PROFILE_MODES.has(profileMode)) throw new Error("Option 'profileMode' has an unsupported value");
    const adblockMode = raw.adblockMode === void 0 ? DEFAULTS.adblockMode : raw.adblockMode;
    if (!ADBLOCK_MODES.has(adblockMode)) throw new Error("Option 'adblockMode' has an unsupported value");
    if (usesMobileRuleBundles(platform) && adblockMode === "full") throw new Error("Option 'adblockMode=full' exceeds the mobile client memory budget");
    const nodeErrorMode = raw.nodeErrorMode === void 0 ? DEFAULTS.nodeErrorMode : raw.nodeErrorMode;
    if (!NODE_ERROR_MODES.has(nodeErrorMode)) throw new Error("Option 'nodeErrorMode' has an unsupported value");
    const options = Object.freeze({
      ...
      autoGroupMode: enumValue(raw, "autoGroupMode"),
      clientChain: enumValue(raw, "clientChain"),
      profileMode,
      adblockMode,
      nodeErrorMode
    });
    platformPolicyPreset(platform === "windows" || platform === "linux" ? "macos" : platform);
    PARSED.add(options);
    return options;
  }
  function isParsedHiddifyOptions(value) {
  ...
  var PLATFORMS2 = /* @__PURE__ */ new Set(["macos", "windows", "iphone", "ipad", "android"]);
  var CHANNELS2 = new Set(FRONTIER_CHANNELS);
  var PROFILE_MODES2 = /* @__PURE__ */ new Set(["light", "diagnostic"]);
  var ADBLOCK_MODES2 = /* @__PURE__ */ new Set(["off", "full"]);
  var NODE_ERROR_MODES2 = /* @__PURE__ */ new Set(["strict", "compatible"]);
  var ALLOWED_KEYS = /* @__PURE__ */ new Set([...REQUIRED_KEYS, ...Object.keys(DEFAULTS2)]);
  var PARSED2 = /* @__PURE__ */ new WeakSet();
  function requiredString2(raw, key) {
    const value = raw[key];
  ...
  function enumValue2(raw, key, defaultValue) {
  ...
  function parseSingBoxOptions(raw) {
    ...
    const platform = requiredString2(raw, "platform");
    if (!PLATFORMS2.has(platform)) throw new Error("Option 'platform' has an unsupported value");
    if (requiredString2(raw, "output") !== "config") throw new Error("Option 'output' must be config");
    if (requiredString2(raw, "type") !== "collection") throw new Error("Option 'type' must be collection");
    ...
    if (typeof profileMode !== "string" || !PROFILE_MODES2.has(profileMode)) throw new Error("Option 'profileMode' has an unsupported value");
    const adblockMode = raw.adblockMode === void 0 ? DEFAULTS2.adblockMode : raw.adblockMode;
    if (typeof adblockMode !== "string" || !ADBLOCK_MODES2.has(adblockMode)) throw new Error("Option 'adblockMode' has an unsupported value");
    ...
    if (typeof nodeErrorMode !== "string" || !NODE_ERROR_MODES2.has(nodeErrorMode)) throw new Error("Option 'nodeErrorMode' has an unsupported value");
    const options = {
      output: "config",
      type: "collection",
      name: validateCollectionName(raw.name, "Option 'name'"),
      subscriptionName: requiredString2(raw, "subscriptionName"),
      platform,
      channel,
      dnsMode: enumValue2(raw, "dnsMode", DEFAULTS2.dnsMode),
      chinaDns: enumValue2(raw, "chinaDns", DEFAULTS2.chinaDns),
      globalDns: enumValue2(raw, "globalDns", DEFAULTS2.globalDns),
      blockMode: enumValue2(raw, "blockMode", DEFAULTS2.blockMode),
      quicMode: enumValue2(raw, "quicMode", DEFAULTS2.quicMode),
      ipv6Mode: enumValue2(
        raw,
        "ipv6Mode",
        ["macos", "iphone", "ipad"].includes(platform) ? "ipv4-only" : DEFAULTS2.ipv6Mode
      ),
      autoGroupMode: enumValue2(raw, "autoGroupMode", DEFAULTS2.autoGroupMode),
      clientChain: enumValue2(raw, "clientChain", DEFAULTS2.clientChain),
      profileMode,
      adblockMode,
      nodeErrorMode
    };
  ...
  function isParsedSingBoxOptions(value) {
    return value !== null && typeof value === "object" && PARSED2.has(value);
  }
  ...
  function requiredString3(node, key) {
    const value = node[key];
    if (typeof value !== "string" || value.length === 0 || value.trim() !== value) {
  ...
  function tlsFields(node, required = false) {
    ...
    if (node["client-fingerprint"] !== void 0) {
      tls.utls = { enabled: true, fingerprint: requiredString3(node, "client-fingerprint") };
    }
    if (reality !== void 0) {
  ...
  function renderWireGuardEndpoint(node) {
    ...
    return {
      ...
      private_key: requiredString3(node, "private-key"),
      peers: endpointPeers
    };
  }
  function renderSingBoxOutbound(node) {
    ...
    switch (protocol2) {
      case "ss":
      case "shadowsocks":
        outbound = { ...base(node, "shadowsocks"), method: requiredString3(node, "cipher"), password: requiredString3(node, "password") };
        if (["tcp", "udp"].includes(node.network)) outbound.network = node.network;
        break;
      case "vmess":
        outbound = { ...base(node, "vmess"), uuid: requiredString3(node, "uuid"), security: node.security ?? node.cipher ?? "auto" };
      ...
      case "snell": {
        ...
        outbound = { ...base(node, "snell"), psk: requiredString3(node, "psk"), version: outputVersion };
        if (["tcp", "udp"].includes(node.network)) outbound.network = node.network;
        if (outputVersion === 4) {
      ...
      case "vless":
        ...
        outbound = { ...base(node, "vless"), uuid: requiredString3(node, "uuid") };
        ...
        break;
      case "trojan":
        outbound = { ...base(node, "trojan"), password: requiredString3(node, "password"), tls: tlsFields(node, true) };
        outbound.transport = transportFields(node);
        break;
      case "anytls":
        validateAnyTlsShape(node);
        outbound = { ...base(node, "anytls"), password: requiredString3(node, "password"), tls: tlsFields(node, true) };
      ...
      case "hysteria2":
      case "hy2":
        outbound = { ...base(node, "hysteria2"), password: requiredString3(node, "password"), tls: tlsFields(node, true) };
        ...
        break;
      case "tuic":
        outbound = { ...base(node, "tuic"), uuid: requiredString3(node, "uuid"), password: requiredString3(node, "password"), tls: tlsFields(node, true) };
      ...
      case "http":
        ...
        break;
      case "ssh":
        outbound = { ...base(node, "ssh"), user: requiredString3(node, "username") };
        setIf(outbound, "password", node.password);
  ...
  function renderSingBoxRuleSets({ ruleBaseUrl, profileMode = "light", adblockMode = "off", platform }) {
    ...
    const sourceBase2 = usesMobileRuleBundles(platform) ? mobileRuleBase(base2) : base2;
    const adblockBase = adblockMode === "full" ? optionalAdblockBase(base2) : null;
    return sources.map((source) => ({
      type: "remote",
      tag: `rule-${source.id}`,
      format: "binary",
      url: `${source.id === "Advertising" || source.id === "Advertising_Domain" ? adblockBase : sourceBase2}/${source.id}.srs`,
  ...
  function validateHiddifyNodeFields(node) {
    ...
    const protocol2 = normalizeProtocol(node.type);
    if (protocol2 === "anytls" && (node["client-metadata"] !== void 0 || node.client_metadata !== void 0)) {
      throw new Error("Hiddify bundled core does not support AnyTLS client_metadata");
    }
    if ((protocol2 === "hysteria2" || protocol2 === "hy2") && node.hop_interval_max !== void 0) {
      throw new Error("Hiddify bundled core does not support Hysteria2 hop_interval_max");
    }
  }
  function renderHiddifyNode(node) {
  ...
  function validateHiddifyConfig(config) {
    if (!config || typeof config !== "object" || Array.isArray(config)) return { valid: false, errors: ["config must be an object"] };
    const errors = [];
    if (config.http_clients !== void 0 || config.route?.default_http_client !== void 0) errors.push("Hiddify does not support HTTP client objects");
    if (config.experimental?.hiddify_next !== void 0) errors.push("Unrecognized Hiddify experimental metadata");
    if (config.experimental?.cache_file?.store_dns !== void 0) errors.push("Hiddify does not support store_dns");
    for (const inbound of config.inbounds ?? []) if (inbound.dns_mode !== void 0 || inbound.dns_address !== void 0) errors.push("Hiddify does not support TUN DNS mode fields");
    for (const rule of config.dns?.rules ?? []) if (["evaluate", "respond"].includes(rule.action) || rule.match_response !== void 0) errors.push("Hiddify does not support DNS answer evaluation");
    for (const rule of config.route?.rule_set ?? []) {
      ...
      if (rule.http_client !== void 0) errors.push("Hiddify remote rule-set cannot use http_client");
    }
    const copy = structuredClone(config);
    const download = "__hiddify-validation-only__";
    copy.http_clients = [{ tag: download, detour: "\u{1F9ED} DNS \u4E0E\u89C4\u5219\u4E0B\u8F7D" }];
  ...
  function sourceBase(value) {
    ...
    const base2 = value.replace(/\/+$/u, "");
    if (base2.endsWith("/hiddify/rules")) return base2;
    if (base2.endsWith("/sing-box/rule-sets")) return base2.replace(/\/sing-box\/rule-sets$/u, "/hiddify/rules");
    throw new Error("Hiddify rule base URL must end in /hiddify/rules");
  }
  function channelOf(base2) {
    const match = /\/(edge|current|previous)\/hiddify\/rules$/u.exec(base2);
    if (!match) throw new Error("Hiddify rule base URL must include a publication channel");
    return match[1];
  }
  function optionalSourceUrl(base2, id) {
    return base2.replace(/\/(edge|current|previous)\/hiddify\/rules$/u, `/optional/adblock-full/$1/hiddify/rules/${id}.json`);
  }
  function renderHiddifyConfig(rawOptions, nodes, rendererOptions = {}) {
    ...
    const base2 = sourceBase(rendererOptions.ruleBaseUrl);
    const singboxBase = base2.replace(/\/hiddify\/rules$/u, "/sing-box/rule-sets");
    ...
    for (const rule of config.route.rule_set) {
      ...
      if (options.adblockMode === "full" && ["Advertising", "Advertising_Domain"].includes(id)) {
        rule.url = optionalSourceUrl(base2, id);
      } else {
        rule.url = rule.url.replace(/\/sing-box\/mobile-rule-sets\//u, "/hiddify/mobile-rules/").replace(/\/sing-box\/rule-sets\//u, "/hiddify/rules/").replace(/\.srs$/u, ".json");
      }
      rule.format = "source";
      rule.download_detour = "\u{1F9ED} DNS \u4E0E\u89C4\u5219\u4E0B\u8F7D";
      delete rule.http_client;
    }
    const mobile = ["iphone", "ipad", "android"].includes(options.platform);
    const probeRoot = `${base2.replace(/\/rules$/u, "")}/${mobile ? "mobile-rules" : "rules"}`;
    const failover = config.outbounds.find(({ tag }) => tag === "\u{1F9ED} \u89C4\u5219\u4E0B\u8F7D\u6545\u969C\u8F6C\u79FB");
    ...
    config.route.rules = config.route.rules.filter((rule) => !(rule.outbound === "DIRECT" && Array.isArray(rule.ip_cidr) && rule.ip_cidr.includes(globalDnsCidr)));
  ...
  // ../../shared/routing/route-intent.js
  var ROUTE_ACTIONS = Object.freeze(["DIRECT", "PROXY", "REJECT"]);
  ...
  // ../../shared/routing/hiddify-decision.js
  var ACTION_TO_OUTBOUND = Object.freeze({ DIRECT: "direct", PROXY: "proxy", REJECT: "block" });
  function hiddifyCapabilityDiagnostics() {
    return Object.freeze({
      core: "hiddify",
      schemaVersion: 1,
  ...
  var HIDDIFY_RULESET_COMPATIBILITY = Object.freeze({
    maxVersion: 4,
  ...
  function logDiagnostics(context, options, nodes, renderFailures) {
    ...
    if (!method) return;
    try {
      method(`[hiddify-config] ${JSON.stringify({ client: "hiddify", platform: options.platform, channel: options.channel, accepted: nodes.length, ruleCount: renderFailures?.ruleCount ?? null, renderFailures, capability: hiddifyCapabilityDiagnostics() })}`);
    } catch {
    }
  }
  async function operator(input, targetPlatform, context = {}) {
    ...
    if (options.nodeErrorMode === "compatible") {
      ...
      assertRenderableNodes(normalized.nodes, "hiddify", renderHiddifyNode);
