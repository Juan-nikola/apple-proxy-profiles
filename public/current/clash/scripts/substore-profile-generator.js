var ClashProfileBundle = (() => {
  ...
  function protocol(names, clients, { requiredFields = [], tls: tls2 = false, clientNames = {} } = {}) {
  ...
  function validateNode(node) {
    ...
    const tls2 = node.tls === true || definition?.tls === true;
    const warnings = tls2 && !hasTlsIdentity(node) ? ["tls-verification-unclear"] : [];
  ...
  function usesClashMobileRuleBundles(platform) {
  ...
  var REQUIRED = ["output", "type", "name", "nodeSubscriptionUrl", "platform"];
  var PLATFORMS = /* @__PURE__ */ new Set(["iphone", "ipad", "macos", "appletv"]);
  var AD_BLOCK = /* @__PURE__ */ new Set(["off", "full"]);
  ...
  function safeUrl(value) {
    if (typeof value !== "string" || value.length === 0 || value.trim() !== value || CONTROL.test(value) || !value.startsWith("https://") || value.includes("#")) {
  ...
  function parseClashOptions(raw) {
    const values = ownRecord(raw);
    const allowed = /* @__PURE__ */ new Set([...REQUIRED, "subscriptionName", "channel", "adblockMode", "dnsMode", "chinaDns", "globalDns", "blockMode", "quicMode", "ipv6Mode", "autoGroupMode", "clientChain"]);
...
`;
  ...
  // render-dns.js
  function renderClashDns(options) {
  ...
  function providerUrl(base, id, adblockMode, platform) {
    const root = adblockMode === "full" && ["Advertising", "Advertising_Domain"].includes(id) ? base.replace(/\/(?:edge|current|previous)$/u, "/optional/adblock-full") : base;
    const directory = usesClashMobileRuleBundles(platform) ? "mobile-rules" : "rules";
    return root + "/clash/" + directory + "/" + id + ".yaml";
  }
  function renderClashRules({ publicBaseUrl, platform = "macos", adblockMode = "off" } = {}) {
  ...
  function copy(target, source, key, output = key) {
    if (own(source, key) && source[key] !== void 0) target[output] = structuredClone(source[key]);
  }
  function common(node, type) {
    if (!node || typeof node !== "object" || typeof node.name !== "string" || !node.name || typeof node.server !== "string" || !node.server || !Number.isInteger(Number(node.port))) {
  ...
  function render(node) {
    ...
    switch (protocol2) {
      case "ss":
      case "shadowsocks":
        result = common(node, "ss");
        copy(result, node, "cipher");
        copy(result, node, "password");
        copy(result, node, "udp");
        break;
      case "ssr":
        result = common(node, "ssr");
        for (const key of ["cipher", "password", "protocol", "obfs", "obfs-param", "protocol-param"]) copy(result, node, key);
        copy(result, node, "udp");
        break;
      case "snell":
        result = common(node, "snell");
        for (const key of ["psk", "version", "reuse", "obfs", "obfs-host"]) copy(result, node, key);
        copy(result, node, "udp");
        break;
      case "vmess":
        result = common(node, "vmess");
        for (const key of ["uuid", "alterId", "cipher", "udp", "packet-encoding"]) copy(result, node, key);
        if (own(node, "alter-id") && !own(node, "alterId")) copy(result, node, "alter-id", "alterId");
        tls(result, node);
        transport(result, node);
        break;
      case "vless":
        result = common(node, "vless");
        for (const key of ["uuid", "flow", "encryption", "udp", "packet-encoding"]) copy(result, node, key);
        tls(result, node);
        transport(result, node);
        break;
      case "trojan":
        result = common(node, "trojan");
        copy(result, node, "password");
        copy(result, node, "udp");
        tls(result, node);
        transport(result, node);
      ...
      case "hysteria2":
      case "hy2":
        result = common(node, "hysteria2");
        copy(result, node, "password");
        for (const key of ["obfs", "obfs-password", "ports", "hop-interval", "up", "down"]) copy(result, node, key);
        tls(result, node);
        break;
      case "tuic":
        result = common(node, "tuic");
        for (const key of ["uuid", "password", "congestion-controller", "udp-relay-mode", "alpn", "port-hopping", "port-hopping-interval"]) copy(result, node, key);
        tls(result, node);
        break;
      case "socks5":
        result = common(node, "socks5");
        for (const key of ["username", "password", "udp"]) copy(result, node, key);
        tls(result, node);
        break;
      case "http":
        result = common(node, node.tls === true ? "https" : "http");
        for (const key of ["username", "password", "headers", "udp"]) copy(result, node, key);
        tls(result, node);
        break;
      case "ssh":
        result = common(node, "ssh");
        for (const key of ["username", "password", "private-key", "host-keys", "tfo"]) copy(result, node, key);
        break;
      case "wireguard":
        result = common(node, "wireguard");
        for (const key of ["private-key", "public-key", "pre-shared-key", "reserved", "ip", "ipv6", "dns", "mtu", "keepalive"]) copy(result, node, key);
        break;
      default:
        throw new Error("Unsupported Clash protocol: " + protocol2);
    }
    if (CHAIN_KEYS.some((key) => own(node, key) && node[key] !== void 0 && node[key] !== null && node[key] !== "")) {
  ...
  function toClashProxy(node) {
    try {
      ...
      throw new Error("Clash cannot render protocol: " + normalizeProtocol(node?.type));
    }
  }

  // validate-profile.js
  var REQUIRED2 = ["proxies", "proxy-groups", "rule-providers", "dns", "rules"];
  function validateClashProfile(profile) {
    ...
    if (!/^rules:\n[\s\S]*MATCH,🚀 节点选择/mu.test(profile)) errors.push("missing terminal MATCH rule");
  ...
  async function operator(input, targetPlatform, context = {}) {
    ...
    const prepared = prepareClashInventory(normalized.nodes, { onDiagnostics(value) {
      renderDiagnostics = value;
    } });
    const content = renderClashProfileFromOptions(options, normalized.nodes, { preparedInventory: prepared });
    const diagnostics = mergedEgernDiagnostics(normalized.diagnostics, renderDiagnostics);
    logEgernDiagnostics(context, { ...diagnostics, client: CLIENT.clash });
    return { ...input, $content: content };
  }
  return __toCommonJS(substore_profile_entry_exports);
})();
async function operator(input, targetPlatform) { return ClashProfileBundle.operator(input, targetPlatform, { arguments: $arguments, produceArtifact, logger: console }); }
