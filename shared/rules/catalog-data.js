function rule(id, policy, minEntries, inputFormat = "RULE-SET", directory = id) {
  return Object.freeze({
    id,
    sourcePath: `${directory}/${id}.list`,
    policy,
    minEntries,
    inputFormat,
  });
}

export const UPSTREAM_RULE_SOURCE_DEFINITIONS = Object.freeze([
  rule("Hijacking", "☣️ 安全威胁", 150),
  rule("BlockHttpDNS", "☣️ 安全威胁", 40),
  rule("Advertising", "🧱 常见广告", 700),
  rule("Advertising_Domain", "🧱 常见广告", 250_000, "DOMAIN-SET", "Advertising"),
  rule("Privacy", "🕵️ 严格跟踪", 15),
  rule("BiliBili", "🇨🇳 国内平台", 80),
  rule("ByteDance", "🇨🇳 国内平台", 300),
  rule("XiaoHongShu", "🇨🇳 国内平台", 3),
  rule("Weibo", "🇨🇳 国内平台", 3),
  rule("OpenAI", "🤖 AI 专用", 20),
  rule("Claude", "🤖 AI 专用", 2),
  rule("Gemini", "🤖 AI 专用", 8),
  rule("Copilot", "🤖 AI 专用", 30),
  rule("GitHub", "🐙 GitHub", 20),
  rule("YouTube", "📺 YouTube", 120),
  rule("Netflix", "🎬 海外流媒体", 800),
  rule("Disney", "🎬 海外流媒体", 100),
  rule("Spotify", "🎬 海外流媒体", 20),
  rule("GlobalMedia", "🎬 海外流媒体", 700),
  rule("Telegram", "💬 海外社交", 25),
  rule("Facebook", "💬 海外社交", 350),
  rule("Instagram", "💬 海外社交", 3),
  rule("Twitter", "💬 海外社交", 20),
  rule("TikTok", "🎬 海外流媒体", 20),
  rule("Apple", "🍎 Apple", 25),
  rule("Microsoft", "🪟 Microsoft", 400),
  rule("SteamCN", "DIRECT", 10),
  rule("ChinaMax_Domain", "DIRECT", 100_000, "DOMAIN-SET", "ChinaMax"),
  rule("Game", "🕹️ 游戏平台", 400),
  rule("Download", "⬇️ 下载/P2P", 5),
  rule("PrivateTracker", "⬇️ 下载/P2P", 150),
  rule("ChinaMax", "DIRECT", 8_000),
]);

/** @deprecated Use UPSTREAM_RULE_SOURCE_DEFINITIONS for upstream fetch inputs. */
export const RULE_SOURCE_DEFINITIONS = UPSTREAM_RULE_SOURCE_DEFINITIONS;
