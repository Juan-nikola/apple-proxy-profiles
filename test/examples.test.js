...
for (const platform of ["macos", "iphone", "ipad"]) {
  test(`${platform} example contains the layered continent selectors`, async () => {
    ...
    assert.match(profile, /^🚀 节点选择 = select,⚡ 全部自动,🛟 全部故障转移,🌏 亚太,🌍 欧洲,🌎 美洲,Shadowrocket-Nodes,use=true,policy-regex-filter=\^\.\+\$$/m);
    assert.match(profile, /^🤖 AI 专用 = select,🤖 AI 亚太,🤖 AI 欧洲,🤖 AI 美洲,Shadowrocket-Nodes,use=true,policy-regex-filter=\^\.\+\$$/m);
    assert.match(profile, /^🤖 AI 亚太 = select,.*use=true,.*hidden=1$/m);
    assert.match(profile, /^🐙 GitHub = select,🚀 节点选择,DIRECT,Shadowrocket-Nodes,use=true,policy-regex-filter=\^\.\+\$$/m);
  });
}
