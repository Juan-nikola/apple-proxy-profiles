...
for (const platform of ["macos", "iphone", "ipad"]) {
  test(`${platform} example contains the layered continent selectors`, async () => {
    ...
    assert.match(profile, /^🚀 节点选择 = select,PROXY,⚡ 全部自动,🛟 全部故障转移,🌏 亚太,🌍 欧洲,🌎 美洲,include-all-proxies=true,policy-regex-filter=\^\.\+\$$/m);
    ...
    assert.match(profile, /^🐙 GitHub = select,🚀 节点选择,DIRECT,include-all-proxies=true,policy-regex-filter=\^\.\+\$$/m);
