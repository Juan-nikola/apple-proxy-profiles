...
test("monorepo exposes all client workspaces and root verification", async () => {
  ...
  await access(new URL("../clients/hiddify/package.json", import.meta.url));
  ...
  assert.equal(lockfile.packages["clients/hiddify"]?.name, "@apple-proxy-profiles/hiddify");
  ...
  assert.equal(root.scripts["verify:hiddify"], "npm --workspace @apple-proxy-profiles/hiddify run verify");
