...
纯 IP、未加密 TCP、ECH 隐藏域名或嗅探失败的请求仍可能只显示 IP，这是协议边界，不代表路由规则失效。macOS 任务会通过 HAPP 官方 `proxy-enable: 1` 参数在导入/更新时自动选择 Proxy 模式，使用 HTTP/HTTPS `127.0.0.1:10809`、SOCKS `127.0.0.1:10808`；可运行 `scutil --proxy` 检查，避免 Surge、Shadowrocket 或其他 VPN 同时接管系统代理。旧订阅需删除后重新导入最新 File URL 才会应用该参数。
