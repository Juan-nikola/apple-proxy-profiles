...
const PLATFORMS = new Set(["macos", "iphone", "ipad", "all"]);
...
const DEFAULTS = Object.freeze({ channel: "current", dnsMode: "stable", chinaDns: "alidns", globalDns: "cloudflare", blockMode: "balanced", quicMode: "proxy-block", ipv6Mode: "auto" });
const REQUIRED = new Set(["output", "type", "name", "subscriptionName", "platform"]);
const ALLOWED = new Set([...REQUIRED, "channel", "dnsMode", "chinaDns", "globalDns", "blockMode", "quicMode", "ipv6Mode"]);
...
export function parseHappOptions(raw) {
  ...
  const options = {
    ...
    platform,
    channel: values.has("channel") && values.get("channel") !== undefined ? values.get("channel") : DEFAULTS.channel,
