const PLATFORM_METADATA = Object.freeze({
  macos: { tun: false, proxy: "desktop" }, iphone: { tun: true, proxy: "network-extension" }, ipad: { tun: true, proxy: "network-extension" }, android: { tun: true, proxy: "vpn-service" }, windows: { tun: true, proxy: "wintun" }, linux: { tun: true, proxy: "tun" },
});
...
export function renderHappInbounds(platform) {
  ...
  const common = { listen: "127.0.0.1", sniffing: { enabled: true, destOverride: ["http", "tls"], routeOnly: true } };
  return [
