...
- **空订阅**：`apple-proxy-sources` 为空，或过滤后没有 Happ 兼容节点。先分别预览来源与组合，确认至少一个可接受节点；不要导入空 JSON。
...
- **不兼容协议**：同名节点存在但不能由 Happ/Xray 使用，审计为 `incompatible-node-fallback`，自动改为 `FOLLOW`；不要伪造协议字段，改用兼容节点。
