...
- **macOS 已连接但应用不走代理**：HAPP macOS 使用本地 HTTP `127.0.0.1:10809` 和 SOCKS `127.0.0.1:10808`。运行 `scutil --proxy`，确认 `HTTPEnable`/`HTTPSEnable` 或 `SOCKSEnable` 与实际端口一致；若都为 `0`，请在系统网络设置中启用代理。Surge、Shadowrocket、Clash、系统 VPN 或其他网络扩展可能覆盖代理设置，排查时只保留一个软件接管系统代理。显式 `curl --socks5-hostname 127.0.0.1:10808 https://www.google.com` 成功而浏览器失败时，问题属于系统代理接管，不是策略路由。
