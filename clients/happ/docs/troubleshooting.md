...
- **日志只显示 IP 或 `sr-*`，看不到域名和节点名**：先确认使用的是新版 HAPP JSON，入口的 `sniffing.destOverride` 包含 `http`、`tls`、`quic` 且 `routeOnly` 为 `false`，日志级别为 `info`。域名只有在 HTTP/TLS/QUIC 嗅探成功时可见；纯 IP、未加密 TCP、ECH 或嗅探失败时显示 IP 属于正常边界。新版出站标签会显示脱敏节点名，例如 `happ-follow/小秘书GEN2 · VLESS · U`，重复名称才会追加短 ID。
