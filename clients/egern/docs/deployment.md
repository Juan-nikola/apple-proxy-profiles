...
Egern 新任务只读取 `apple-proxy-egern`。先按 [Sub-Store 客户端节点池指南](../../../docs/substore-client-pools.md) 完成筛选、preview 和回滚准备；已有 `apple-proxy-sources` collection、tasks 和旧 URL 保留作兼容/回滚。
...
- 参数原样复制：`output=nodes&type=collection&name=apple-proxy-egern&clientChain=off`
- 来源类型是已有 collection，名称必须仍是 `apple-proxy-egern`。
