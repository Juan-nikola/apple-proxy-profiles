...
function enumValue(values, key, allowed, defaultValue) {
  const value = values.has(key) && values.get(key) !== undefined ? values.get(key) : defaultValue;
...
function chainTarget(values) {
  const value = values.has("clientChainTarget") && values.get("clientChainTarget") !== undefined
