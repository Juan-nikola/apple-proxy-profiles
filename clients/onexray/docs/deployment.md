...
2. 先安装 `geosite.dat` 和 `geoip.dat`，再导入私密 Profile；Profile 与 GeoData 必须来自同一份 `current` Manifest。
3. 生产直接使用已通过自动化门禁的唯一公开 `current`；真机 canary 是上线后的可选反馈，不阻塞 promotion。
4. 在设备上保留配套的旧 Profile 和 GeoData，作为成对回滚依赖。
