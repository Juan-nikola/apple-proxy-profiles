...
```text
<SUBSTORE_API>/api/file/onexray-nodes
```

`/subs?api=...` 是网页管理入口，返回 HTML，不能作为 OneXray 节点订阅。
...
```text
.../onexray-profile-generator.js?v=10#...&policyFile=onexray-policy&logLevel=info&dnsLog=on
```
