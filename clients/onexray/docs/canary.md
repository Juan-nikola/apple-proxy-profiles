...
| 平台 | GeoData | Profile 导入 | Rule 模式 | DNS/路由 | IPv4/IPv6/QUIC | 固定节点故障 | 本地旧 Profile/GeoData 回滚 | 状态 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
每个平台如需反馈，可记录应用版本、current Profile/GeoData hash、主节点切换、DIRECT/FOLLOW/NODE、DNS、网络切换、固定节点失败与恢复，以及本地旧 Profile/GeoData 回滚结果。设备反馈只用于定位和改进，不决定客户端 promotion。
