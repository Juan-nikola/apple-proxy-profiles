...
function actionOutbound(rule) {
  return rule?.action === "route" || rule?.action === "bypass" ? rule.outbound : undefined;
}
...
function validateDnsServerShape(server, errors) {
  ...
  if (server.server_port !== undefined && (!Number.isInteger(server.server_port) || server.server_port < 1 || server.server_port > 65535)) {
    errors.push("HTTPS DNS server_port must be between 1 and 65535");
  }
  if (server.path !== undefined && (typeof server.path !== "string" || !server.path.startsWith("/") || /[\r\n]/u.test(server.path))) {
...
function validateDnsRule(rule, dnsServers, ruleSets, outboundTags, evaluateTags, errors) {
  ...
  if (rule.action === "respond" && rule.server !== undefined) errors.push("DNS respond must not specify a server");
  if (rule.action === "evaluate" && rule.tag !== undefined) {
    if (typeof rule.tag !== "string" || evaluateTags.has(rule.tag)) errors.push("duplicate DNS evaluate tag");
    else evaluateTags.add(rule.tag);
  }
  if (rule.match_response !== undefined) {
  ...
  if (rule.ip_accept_any === true && rule.match_response === undefined) errors.push("DNS ip_accept_any requires match_response");
...
export function validateSingBoxConfig(config) {
  ...
  for (const client of config.http_clients ?? []) {
    if (client.detour !== undefined && !outboundTags.has(client.detour)) errors.push("HTTP client references missing outbound tag");
  }
  ...
  for (const rule of routeRules ?? []) {
    ...
    if (rule.action === "resolve" && rule.server !== undefined && (typeof rule.server !== "string" || !dnsServers.has(rule.server))) {
  ...
  if (config.route?.default_domain_resolver !== undefined) {
    const resolver = typeof config.route.default_domain_resolver === "string"
