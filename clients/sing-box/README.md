...
3. `.cn` 等中国域名直连。
4. 其他域名执行 `resolve`，用 `ChinaIP` rule-set 判断解析结果；中国 IP 直连，其他地址进入 `漏网之鱼`。
...
- `🤖 AI 专用`、`🐙 GitHub`、`📺 YouTube`、`🎬 海外流媒体`、`💬 海外社交`、`🍎 Apple`、`🪟 Microsoft`、`🇨🇳 国内平台`、`🌍 海外游戏`：统一业务策略组；另有 `🎮 游戏连接`、`⬇️ 下载/P2P`、`🧭 DNS 与规则下载`、`漏网之鱼` 和三类安全组。各业务组保留首页跟随、自动测速、各洲组和手动节点候选；自动测速组排在手动组之后。
- `🧭 DNS 与规则下载`：默认使用直接探测规则文件的 `🧭 规则下载故障转移` URLTest，并保留手动节点和 `DIRECT`；代理 DNS 永远绕过它并经 `⚡ 全部自动`，避免启动环路。

sing-box 不生成伪造的 fallback 组。规则下载专用 `urltest` 只负责根据实际规则文件选择健康出口，不是有序请求重试；如果所有候选都不可用，规则集初始化仍会失败。

完整配置任务读取私密 `apple-proxy-policy`，节点任务只输出节点。`final=FOLLOW`、`DIRECT`、`NODE~查询词` 控制 `漏网之鱼` 的默认值；它始终包含 `🚀 节点选择`、`DIRECT`、`REJECT`，其中 `REJECT` 仅手动使用。固定节点不存在或不兼容时任务会失败。
