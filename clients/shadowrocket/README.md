...
1. 私密节点订阅：File `shadowrocket-nodes` 引用排序节点生成器，从 `apple-proxy-shadowrocket` 读取节点；输出含节点凭据，只在自己的 Sub-Store 与设备之间使用。
2. `shadowrocket-config-*`：macOS、iPhone、iPad 的配置 Profile，不含节点凭据。

第一次使用先读根目录的 [Sub-Store 外置 JS + 任务引用总指南](../../docs/substore-two-layer-setup.md)，再严格按 [零基础部署手册](docs/deployment.md) 操作：节点 File 和三个 Profile File 都选择链接模式引用各自的 Pages JS URL，并在任务自己的参数编辑器中填写参数。日常增加节点或切换 DNS、QUIC、IPv6 时看 [维护速查](docs/maintenance.md)。出现网络、局域网、AI、评论地区或更新异常时看 [故障排查与回滚](docs/troubleshooting.md)。
...
仓库在 GitHub Pages 维护两条 Shadowrocket 外置 JS。Sub-Store 不需要先创建独立脚本记录；后面的 File 直接引用对应 URL。
...
先创建节点 File `shadowrocket-nodes`，选择“链接/远程脚本”并引用节点生成器；再创建 3 个 Profile File，引用 Profile 生成器。四个 File 都在自己的可视化参数编辑器中填写参数，不粘贴 JavaScript 正文。
...
- `clients/shadowrocket/dist/shadowrocket-node-subscription.js`：发布到规范 Pages URL，由 `shadowrocket-nodes` File 以链接模式直接引用。
...
下面按 Sub-Store 常见界面的用途写步骤；如果按钮名称略有差异，就寻找“组合订阅”“文件/File”“链接/远程脚本”和“参数/Arguments”这些同用途入口。Sub-Store 不需要先创建独立脚本记录：节点 File 和三个 Profile File 直接引用稳定的公开 JavaScript URL，参数只保存在各自任务中，不把脚本正文复制进任务。
...
2. 找到原始组合 `apple-proxy-shadowrocket`。如果还没有，就新建同名组合，并加入你已单独预览并确认可用的私密来源。
...
1. 在 Sub-Store 新建 File `shadowrocket-nodes`，添加启用且参与预览的远程脚本操作：`https://juan-nikola.github.io/apple-proxy-profiles/current/shadowrocket/scripts/shadowrocket-node-subscription.js`。
2. 参数填写 `output=nodes&type=collection&name=apple-proxy-shadowrocket&clientChain=off`；旧版单行界面使用 `JS_URL#参数`，不要使用 `?`。
3. 预览至少应返回一个经过统一命名和排序的节点。保存 File 后复制它的私密输出 URL，在 Shadowrocket 中命名为 `Shadowrocket-Nodes`（或自己的显示名），建议每 6 小时更新。
4. 私密订阅 URL 只保存在自己的设备中；不要把它粘贴回仓库或文档。

成功标志：`shadowrocket-nodes` 预览非空；生成的节点名称带统一来源标签，国旗不重复，日志只出现聚合计数，不出现服务器、UUID 或密码。
...
4. 先预览 macOS Profile，确认整行是 `🚀 节点选择 = select,PROXY,Shadowrocket-Nodes,use=true,policy-regex-filter=^(?!🔗 ).+$`；常用业务组都有自动测速/地区/具体节点选择，自动测速和各洲手动节点已保留在业务分组中，境外组首项为 `🚀 节点选择`，国内组首项为 `DIRECT`；动态组只含与 `subscriptionName` 完全匹配的 `<subscriptionName>,use=true`，`🤖 AI 专用`不再包含洲组，再发布并只在 Intel Mac 更新测试。
...
Shadowrocket 可能保留其他业务分组中仍然有效的旧选择，生成时的首项默认值不会自动覆盖它。逐个查看常用业务组：境外组希望跟随首页时，手动选择第一项 `🚀 节点选择`；国内组希望恢复默认直连时，手动选择第一项 `DIRECT`。如果摘要仍显示具体节点、自动测速或地区组，业务会继续按该旧选择工作。
