...
- 新增机场：加入 `apple-proxy-sources`，名称以 `[机场]` 开头，不改生成器源码。
...
2. 先预览该来源，再预览 `apple-proxy-sources`，最后预览 `Shadowrocket-Nodes`。
...
1. 保持正式的 `apple-proxy-sources`、参数为 `output=nodes&clientChain=off` 的节点 Script Operator、`Shadowrocket-Nodes`、三个正式 Profile File 及其 URL 全部不变。
