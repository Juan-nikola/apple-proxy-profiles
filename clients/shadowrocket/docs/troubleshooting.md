...
在 `🤖 AI 专用`固定一个稳定具体节点，不频繁更换；核对账号地区、付款地区和服务条款。代理只能改变网络出口，不能保证解除账号风控。

先看规则日志是否命中 `🤖 AI 专用`，再检查所选具体节点是否可用。不要为了绕过账号限制伪造身份或频繁切换出口。

## 评论地区没有变化

在 `🇨🇳 国内平台` 组内切换 DIRECT、`🚀 节点选择`或具体节点，确认规则日志命中该组。即使出口已改变，账号、手机号、GPS、缓存和平台风控仍可能决定评论地区；本项目不伪造 GPS，也不承诺显示变化。
