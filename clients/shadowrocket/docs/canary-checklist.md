...
- [ ] 已在脚本管理中更新共享记录 `shadowrocket-profile-generator.js` 一次，并重新运行当前平台 File、更新 Profile；三个 File 不重复粘贴脚本，任务名、私密 URL和参数保持不动。既有共享记录若仍引用旧 `substore-profile-generator.js` 兼容 URL，可保持原 URL，只需确认内容已更新。设备端无需替换节点组合 Operator 或改动节点订阅。仓库内 `clients/shadowrocket/dist/`、`clients/shadowrocket/examples/` 已重建校验，节点 Operator 内容未改变；规则的唯一批准变更是用完整 `Advertising` 取代 `AdvertisingLite`，同时引用 `Advertising.list` 与 `Advertising_Domain.list`。
