...
import { xrayGeoCode as oneXrayGeoCode, xrayGeoNames as oneXrayGeoNames, xrayGeoReference as oneXrayGeoReference } from "../../../shared/xray-geodata-contract.js";
...
function geoReferences(geoData, options) {
  ...
  if (geoData === null || geoData === undefined) {
    const sources = sourcesForRegion(options.region).map((id) => ({ id, code: oneXrayGeoCode(id) }));
    return {
      sources,
      domain: sources.map(({ id }) => oneXrayGeoReference(options.channel, "domain", id)),
      ip: sources.map(({ id }) => oneXrayGeoReference(options.channel, "ip", id)),
    };
  }
...
export function renderV2rayNProfile({ nodes, options, geoData = null, filterFailures = {}, policyResolution = null } = {}) {
  ...
  const sourceRules = references.sources.map((source) => ({ source, outboundTag: actionForSource(source.id, overrides, nodeTags, nodeTagsById, options.blockMode, policyResolution, proxyTag) }));
