...
const options = parseV2rayNOptions({ output: "config", type: "collection", name: "v2rayn-fixture", platform: "windows" }); await mkdir(resolve(root, "examples"), { recursive: true });
await writeFile(resolve(root, "examples/v2rayn-profile-windows.json"), `${JSON.stringify(renderV2rayNProfile({ nodes, options }), null, 2)}\n`); await writeFile(resolve(root, "examples/v2rayn-nodes.json"), renderV2rayNSubscription({ nodes }));
