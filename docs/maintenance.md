...
Sub-Store 维护 10 个手动 collection、38 个 canonical task，不使用 `subscriptionTags`。HAPP 的 GeoData 位于 `public/current/happ/geoip.dat` 和 `public/current/happ/geosite.dat`；V2Box 的 GeoData 位于 `public/current/geodata/<region>/`，URL 与文件名必须保持稳定。
