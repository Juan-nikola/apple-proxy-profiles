...
Sub-Store 当前的九个 active 客户端 collection 边界、迁移和回滚见 [Sub-Store 客户端节点池指南](substore-client-pools.md)。HAPP、OneXray、v2rayN 和 V2Box 已接入原生 renderer、GeoData、公开安装页和发布闭包；节点、Profile 与策略仍只在私密任务中生成。旧 `apple-proxy-sources` 保留作兼容/回滚入口。
...
| Sub-Store 客户端文档 | 已完成 | `apple-proxy-all`、9 个 client collection、公开 JS、34 个任务、维护/编译/回滚指南 | 文档测试、秘密扫描 |
| GitHub Pages 公开发布 | edge 已完成，current 待批准 | 九个 active 客户端的无凭据 GeoData、安装页和私密 renderer 已生成并进入 `edge/` 候选；现有 `current/` 与 `previous/` 仍保持七客户端稳定快照，待审核后按客户端 manifest hash 显式晋级 | Actions 检查、edge/current/previous 闭包与秘密扫描 |
...
| 私密 Sub-Store 任务 | 已配置 | 10 个 collection、34 个任务模板已生成；稳定模板使用 `current`，新客户端在晋级前使用同构的 `edge` URL 预览，旧入口保留作兼容/回滚 | canonical 配置校验、任务 URL 检查、公开脚本验证 |
