...
| GitHub Pages 公开发布 | 已完成 | 既有客户端使用 `current/`；HAPP/OneXray 已生成无凭据 `edge/` 候选、GeoData 和安装页，待 canary 后再提升 `current/` | Actions 检查、edge 脚本闭包与秘密扫描 |
...
| 私密 Sub-Store 任务 | 待用户迁移 | 保留旧 collection/tasks，按 canonical 指南建立总池与七个 client collection，再逐客户端切换 `name=` | preview、refresh/计数对比、旧 URL 回滚演练 |
