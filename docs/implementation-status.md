...
| Sub-Store 客户端文档 | 已完成 | `apple-proxy-all`、9 个 client collection、公开 JS、35 个任务、维护/编译/回滚指南 | 文档测试、秘密扫描 |
| GitHub Pages 公开发布 | 已完成 | 九个 active 客户端的无凭据 GeoData、安装页和私密 renderer 已通过自动化门禁并发布到 `current/`；`edge/` 保留为隔离预览，`previous/` 保留为回滚快照 | Actions 检查、edge/current/previous 闭包与秘密扫描 |
...
| 私密 Sub-Store 任务 | 已配置 | 10 个 collection、35 个任务已配置；正式任务使用 `current`，维护者可用同构的 `edge` URL 做隔离预览，旧入口保留作兼容/回滚 | canonical 配置校验、任务 URL 检查、公开脚本验证 |
