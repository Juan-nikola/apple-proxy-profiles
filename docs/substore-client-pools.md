...
Sub-Store 由用户自己保存节点来源和筛选结果。仓库只提供公开 renderer，不接触你的私密订阅地址。当前维护 8 个 collection：一个总池和七个客户端池。
...
5. 验证完成后，手动删除旧 collection `apple-proxy-happ`、`apple-proxy-onexray`、`apple-proxy-v2rayn` 及其 13 个旧 File task；仓库不会再生成这些入口。
