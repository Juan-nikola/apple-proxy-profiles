...
`apple-proxy-all` 是用户在自己 Sub-Store 中建立的总池，用于汇总用户自己选择的来源。它不是仓库发布物，也不是六个客户端任务的直接输入。六个客户端分别读取下列 client collection：
...
用户必须在 Sub-Store 中自行选择每个 client collection 的成员。生成器不做客户端能力白名单过滤：勾选什么节点就处理什么节点；渲染器确实无法表示的协议会跳过，并按协议计数写入 `renderFailures` 诊断，不会静默丢弃。只有没有任何节点可渲染时任务才失败。先看 preview 日志里的 `renderFailures`，再决定是否修正 collection 选择。
...
4. **用户自行筛选**：直接编辑对应 collection 选择要包含的节点；生成器不按客户端能力白名单过滤，渲染器无法表示的已选协议会跳过并计入 `renderFailures`，全部不可渲染才失败。
...
`apple-proxy-sources` 是保留的兼容/回滚路径，不是新部署的六客户端共享输入。迁移期间保留该 collection、所有旧 tasks 与私密旧 URL。若新任务 preview、refresh、计数对比或客户端灰度任一失败，立即在该客户端切回旧 URL，无需更改其他客户端。

## 协议标签与 OneXray 精确名称迁移

节点规范化后，新名称使用 `· <Protocol>` 协议标签。OneXray 中固定的 `NODE:<name>` 目标以及任何外部 exact-name 引用都不会自动跟随改名：先 preview 对应 client collection，确认规范化后的新名称，再把旧名称引用更新为带 `· <Protocol>` 的新名称。更新后重新生成 Profile 与审计输出；若固定目标无法解析，保持旧 URL 并停止迁移。
