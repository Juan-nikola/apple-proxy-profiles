...
11 个手动 collection：`apple-proxy-all`、`apple-proxy-egern`、`apple-proxy-anywhere`、`apple-proxy-shadowrocket`、`apple-proxy-surge`、`apple-proxy-singbox`、`apple-proxy-happ`、`apple-proxy-v2rayn`、`apple-proxy-v2box`、`apple-proxy-clash`、`apple-proxy-incy`。

## 脚本与任务

canonical catalog 共 43 个 File task。每个 generator task 都是“File + Script Operator”：File 源内容留空，Script Operator 以远程链接引用下面的 JS；不要把 JS 放到 File 的远程源文件字段。配置任务读取 schema v3 按客户端分层的 policy，节点任务不读取 policy。典型路径：
...
以下参数由各客户端生成器共同使用或按客户端扩展。参数名和值区分大小写；只填写表中的值。当前私密 Sub-Store 的 34 个配置任务已统一设置为 `ipv6Mode=ipv4-only`，以避免无可用 IPv6 路由时出现连接失败。
...
| `ipv6Mode` | IPv4/IPv6 解析和连接策略 | `auto`、`ipv4-only` | 所有 34 个配置任务 `ipv4-only` |
...
单独建立一个私密 File task，内容输出名为 `apple-proxy-policy`。使用 schema v3：顶层包含 10 个客户端层，每层嵌套 schema v2，并完整填写以下 13 个业务目标。业务组显示名由生成器固定为中文，不要在各客户端另建英文同名组：
...
先 Preview 11 个 collection 和 43 个任务，再逐个替换客户端订阅。失败时切换设备上的旧 Profile/Config；HAPP 固定节点运行时仅允许 balancer 回退 FOLLOW。后台删除 OneXray 遗留对象及旧任务，保留新的 `apple-proxy-happ` collection。
