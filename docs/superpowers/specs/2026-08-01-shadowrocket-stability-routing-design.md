...
在不改变现有业务分组结构和各分组可选项语义的前提下，解决以下问题：
...
- 保留全部现有策略组。
- 除“🚀 节点选择”新增首页跟随入口外，现有业务分组的默认顺序和手动候选语义不变。
...
Shadowrocket 内建策略 `PROXY` 表示首页当前选择的节点。把它放在“🚀 节点选择”的第一个候选即可实现默认跟随首页，同时保留自动、故障转移、洲组和具体节点。
...
- “🚀 节点选择”候选顺序改为：`PROXY`、全部自动、全部故障转移、现有洲组、按过滤器得到的具体节点。
- 其余业务组继续保持原来的 DIRECT/“🚀 节点选择”先后顺序，并继续提供具体节点候选。
...
- 不自动改变用户已经保存的策略组选项；新增的“🚀 节点选择”首项只决定新建/重建组的默认选择。若客户端保留旧选项，用户可手动选择一次 `PROXY`。
