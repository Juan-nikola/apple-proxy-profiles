...
- The current official Egern proxy reference explicitly documents Shadowsocks, Snell, Trojan, AnyTLS, Hysteria2, TUIC, SOCKS5, SSH, HTTP/HTTPS, VMess, VLESS, and WireGuard: <https://egernapp.com/docs/configuration/proxies/>.
- The foundation registry already admits VMess, HTTP, and WireGuard for Egern. Task 2 must map and test those protocols in addition to the originally enumerated fixtures, and the 2026-08-02 preflight must add current official SSH support to the registry; it must not silently filter documented, losslessly representable nodes after the shared capability gate accepts them.
...
- SSH maps to Egern `ssh` with required `username`, at least one of inline `private_key` or `password`, and verified optional `host_keys`. It remains unsupported by Shadowrocket and Anywhere.
...
Create fake reserved-address fixtures for Shadowsocks 2022, Snell v4, VMess, VLESS TCP+Reality, VLESS WSS, VLESS gRPC+Reality, Trojan, AnyTLS, Hysteria2, TUIC, SOCKS5, SSH, HTTP/HTTPS, WireGuard, and unsupported transport/multi-peer cases.
...
- `ssh` → `ssh` with required `username`, password and/or inline private key authentication, and optional pinned host keys.
