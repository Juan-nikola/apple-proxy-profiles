...
Reorder `DEFAULT_RULE_SOURCE_IDS` to the same canonical phase order so the compiler, publication catalog and Anywhere priority all agree:
...
Use the existing custom-rule loops and existing rule-download host lines unchanged; replace only the source selection around them:

```js
const plan = orderedRoutingPlan({ adblockMode });
lines.push(...plan.filter(({ phase }) => phase === "security").map(render));

// Keep the renderer's existing custom-rule loop here.
// Keep the renderer's existing rule-download bootstrap DOMAIN line here.
...
The two comments identify existing code that remains byte-for-byte present; they are not new helper functions. Keep native GEOIP CN and final rules after the shared plan.
...
[
  ...
  { domain: "unknown.example", ip: "8.8.8.8", expectedSource: null, expectedPolicy: "🚀 节点选择" },
]
```
...
```bash
npm run update:rules -- --promote "$APPROVED_CLIENT" "$APPROVED_MANIFEST_HASH"
```

Set `APPROVED_CLIENT` and `APPROVED_MANIFEST_HASH` from the exact user-approved canary record in the execution environment immediately before running the command. The promotion validator must reuse the edge audit report and reject stale or blocked evidence.
...
Use one workflow-created commit per approved client. Verify its message is exactly one of the five allowed forms:

```text
chore: promote tested client shadowrocket
chore: promote tested client surge
chore: promote tested client egern
chore: promote tested client anywhere
chore: promote tested client singbox
```

Only the forms corresponding to explicitly approved clients may appear; no unapproved client may change.
